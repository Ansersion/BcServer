...
if ( __name__ == "__main__" ):
    ...
    try:
        with open('clients_pid.dat', mode='w') as file:
            ...
            pro = subprocess.Popen([Handler.device_bin, Handler.host, Handler.device_sn, Handler.device_pwd, Handler.ping_interval, Handler.device_output_file])
            file.write("Device: " + str(pro.pid) + "\r\n")
            time.sleep(3)

            # create the user client
            pro = subprocess.Popen([Handler.user_bin, Handler.host, Handler.user_id, hashlib.sha256(Handler.user_pwd.encode("utf-8")).hexdigest(), Handler.post_interval, Handler.dev_id, Handler.user_output_file])
