...
public class BcServerHandler extends IoHandlerAdapter {
    ...
    public void messageReceived(IoSession session, Object message ) throws Exception
    {
    	...
    	if(BPPacketType.CONNECT == pack_type) {
        	...
        	if(!UserDB.ChkUserName(user_name)) {
        		System.out.println("Invalid user name:" + user_name);
        		return;
        	}
        	if(!UserDB.ChkUserPwd(user_name, password)) {
        	...
        	/* update login flags */
        	BPSession newBPSession = new BPSession(user_name.getBytes(), password, client_id, user_login_flag, dev_login_flag);
    		...
    		/* if (so) {
    		 * 	push command to get the latest data 
    		 * 
    		 * }
    		 * 	return current data;
    		 * }
    		...
    		/* update the device ID map */
           	...
           	/* if(so) {
           	 * 	update the client ID database
           	 * }
           	 * close the socket
           	 */
    	} else {
    		throw new Exception("Error: messageRecevied: Not supported packet type");
    	}
